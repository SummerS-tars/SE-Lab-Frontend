...
const hrefQuery = (item) => {
  if (item.answer)
    return { answerId: item.answer.answerId };
  if (item.comment)
    return { answerId: item.comment.answerId };
  if (item.reply)
    return { commentId: item.reply.commentId };
};
		...
		<li v-for="(item,index) in tableData" :key="index" style="list-style: none;" >
			...
			<el-card class="message-card">
				<div v-if="item" class="message-content">
					<!-- 用户信息 -->
					<div class="user-info">
					<a :href="`/user/profile/${item.userId}`" class="username">
						{{ item.username }}
					</a>
					点赞了你的
					<router-link
